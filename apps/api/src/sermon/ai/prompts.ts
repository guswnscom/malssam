...
## 문체 규칙
- 세 대지의 전개 방식을 서로 다르게 하세요
- 모든 문단을 같은 길이로 쓰지 마세요
...
const DEPTH_DESC: Record<string, string> = {
  BRIEF: '간결하게 작성해주세요. 전체 약 2,000자 이내.',
  MODERATE: '보통 깊이로 작성해주세요. 전체 약 3,000자.',
  DEEP: '심층적으로 작성해주세요. 전체 약 5,000자. 원어적, 역사적 배경을 포함해주세요.',
};
...
## 중요: 성경 본문 정확성
반드시 아래 지정된 성경 본문만 사용하세요.
다른 책이나 다른 장절로 변경하지 마세요.
JSON 응답의 "scripture" 필드에는 반드시 "${input.scripture}"을 그대로 입력하세요.
설교 전체가 이 본문을 중심으로 전개되어야 합니다.
